    ...
    <div className="w-full rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6 md:p-8">
      ...
      <div className="mb-8">
        ...
        <div className="flex justify-between mt-2 text-[10px] font-mono text-zinc-600">
          <span>Job opens. No owner.</span><span>Spreadsheet check</span><span>WhatsApp approval</span><span>Manager on leave</span><span>Candidate gone</span>
        </div>
      </div>
      <div>
        <div className="flex items-baseline justify-between mb-2">
...
function CostSavings() {
  const tools = [
    ...
    { name: "Scheduling apps",    range: "$2,500 – $3,500/yr"   },
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-20 border-t border-zinc-900">
      ...
      <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-3 max-w-3xl">
        Stop paying for 7 tools that don't talk to each other.
      </h2>
      <p className="text-zinc-400 max-w-2xl leading-relaxed mb-10">
        The average 10-recruiter staffing agency spends <span className="text-white font-semibold">$65,000–$90,000 per year</span> running disconnected tools.
        Before implementation. Before integrations. Before the time your team loses switching between them.
      </p>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-6">
            ...
            <p className="text-white text-lg font-semibold leading-snug">
              One system. One subscription. Time-to-hire drops from 14–30 days to 72 hours.
            </p>
          </div>
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/30 p-6 space-y-3">
            ...
            <p className="text-zinc-400 text-sm leading-relaxed">
              At a 10-recruiter agency, that's <span className="text-white font-semibold">60–100 hours of productivity lost every single week</span> — before you even count the candidates who accepted offers elsewhere while you were waiting on a WhatsApp reply.
            </p>
          </div>
        </div>
      </div>
...
const TIERS = [
  {
    name: "Starter",
    sub: "Up to 5 recruiters",
    monthly: 1500,
    features: [
      "Full recruitment workflow automation",
      "Xavier AI candidate scoring (0–100)",
      "Interview routing & scheduling",
      "Real-time in-app notifications",
      "Auto-decline with branded comms",
      "Candidate compliance portal",
    ],
  },
  {
    name: "Professional",
    ...
    features: [
      ...
      "Advanced compliance document tracking",
      "Multi-channel candidate communications",
      "Performance dashboards & analytics",
      "Employee clock in/out with geolocation",
      "Onboarding auto-trigger on hire",
    ],
  },
  {
    name: "Enterprise",
    sub: "Multi-location or compliance-heavy",
    monthly: 6000,
    features: [
      "Everything in Professional",
      "Dedicated implementation & onboarding",
      "Custom integrations & API access",
      "Priority support & SLA agreement",
      "Advanced access controls & audit trail",
    ...
    <section id="pricing" className="max-w-6xl mx-auto px-6 py-20 border-t border-zinc-900">
      ...
      <div className="flex flex-wrap items-end justify-between gap-6 mb-10">
        <h2 className="text-3xl md:text-4xl font-bold tracking-tight max-w-xl">
          Priced for the team you're running today.
        </h2>
        <div className="flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-900/50 p-1">
          <button onClick={() => setAnnual(false)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition ${!annual ? "bg-white text-zinc-900" : "text-zinc-400 hover:text-white"}`}>
            Monthly
          </button>
          <button onClick={() => setAnnual(true)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition flex items-center gap-2 ${annual ? "bg-white text-zinc-900" : "text-zinc-400 hover:text-white"}`}>
            Annual
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {TIERS.map((t) => {
          ...
          return (
            <div key={t.name}
              className={`rounded-2xl p-7 flex flex-col relative ${t.highlight ? "border-2 border-emerald-500 bg-emerald-500/[0.04]" : "border border-zinc-800"}`}>
              {t.highlight && (
                <span className="absolute -top-3 left-6 bg-emerald-500 text-zinc-950 text-[11px] font-bold px-3 py-1 rounded-full">
                  Most popular
                </span>
              )}
              ...
              <button
                ...
                className={`mt-7 w-full py-3 rounded-xl text-sm font-bold transition ${
                  t.highlight
...
const XAVIER_KNOWLEDGE = `You are Xavier, PivotOps's AI advisor on the landing page. You help prospects understand the product and choose the right plan. Be direct, confident, and helpful. Never be salesy or pushy. Answer honestly.

PivotOps is a workforce operations platform that automates the entire hiring-to-day-one workflow: application intake, AI scoring (0-100), interview routing, onboarding, team communication, compliance tracking, and employee clock in/out.

Key facts:
- Time-to-hire: compresses 14-30 days to 72 hours
- Replaces: ATS/CRM, HR tools, HRIS, scheduling apps, ops tracking, team comms — saving $65,000-$90,000/yr for a 10-recruiter agency
- Built for: staffing agencies, workforce-heavy teams, healthcare staffing, merchant/retail operations
- Not built for: pure systems of record, very small teams with no hiring volume

Pricing:
- Starter: $1,500/month (up to 5 recruiters) — full recruitment automation, Xavier AI scoring, interview routing, notifications
- Professional: $2,500/month (5-20 recruiters) — everything in Starter + compliance tracking, multi-channel comms, dashboards, geolocation clock in/out
- Enterprise: $6,000/month (multi-location or compliance-heavy) — everything in Professional + dedicated onboarding, custom integrations, priority support
- Annual plans save 10%

Implementation: High-touch pilots mapped to your actual pipeline. No migration required on day one.

When helping someone choose a plan:
- Ask how many recruiters they have
- Ask if they need compliance document tracking (if yes, Professional or above)
- Ask if they're multi-location (if yes, Enterprise)
- Starter is best for small agencies just getting started
- Professional is the most popular for growing agencies
- Enterprise is for compliance-heavy or multi-location operations

Always end responses with a clear next step: either suggest a plan or recommend they book a demo at /login`;
...
async function askXavier(messages: { role: string; content: string }[]): Promise<string> {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1000,
      system: XAVIER_KNOWLEDGE,
      messages: messages.map(m => ({ role: m.role, content: m.content })),
    }),
  });
  const data = await res.json();
  return data.content?.[0]?.text ?? "I couldn't process that. Try asking again.";
}
...
function XavierChat() {
  ...
  const [messages, setMessages] = useState<{ role: string; content: string; id: number }[]>([
    { role: "assistant", content: "Hi, I'm Xavier — PivotOps's AI advisor. Ask me anything about the platform, pricing, or whether it's the right fit for your team.", id: 0 }
  ]);
  ...
  const send = async (text: string) => {
    ...
    const history = [...messages, userMsg].filter(m => m.role !== "assistant" || m.id === 0 ? false : true)
      .concat(userMsg)
      .map(m => ({ role: m.role, content: m.content }));

    // Build conversation for API (exclude the greeting, include all subsequent)
  ...
  if (!open) return (
    <button
      ...
      className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-4 py-3 rounded-2xl
                 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-semibold text-sm
                 shadow-2xl shadow-emerald-500/30 transition">
      <span className="text-lg">⚡</span>
      Ask Xavier
      <span className="w-2 h-2 rounded-full bg-zinc-950/40 animate-pulse" />
    </button>
  );

  return (
    <div className="fixed bottom-6 right-6 z-50 w-[360px] max-h-[600px] flex flex-col
                    rounded-2xl border border-zinc-700 bg-zinc-950 shadow-2xl shadow-black/60 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 bg-zinc-900/80">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-400">PivotOps advisor · online</p>
          </div>
        </div>
        <button onClick={() => setOpen(false)} className="text-zinc-600 hover:text-white transition text-lg leading-none">×</button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0 max-h-[380px]">
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-zinc-800 rounded-2xl rounded-bl-sm px-3.5 py-2.5 flex gap-1">
              {[0,1,2].map(i => (
      ...
      <div className="px-4 pb-2 flex gap-2">
        <button onClick={() => router.push("/login?mode=signup")}
          className="flex-1 py-1.5 rounded-lg bg-emerald-500/15 border border-emerald-500/25
                     text-emerald-400 text-xs font-semibold hover:bg-emerald-500/25 transition">
          Get started
        </button>
        <button onClick={() => document.getElementById("pricing")?.scrollIntoView({ behavior: "smooth" })}
          className="flex-1 py-1.5 rounded-lg bg-zinc-800 border border-zinc-700
                     text-zinc-400 text-xs font-semibold hover:bg-zinc-700 transition">
          View pricing
        </button>
      </div>
      ...
      <div className="p-3 border-t border-zinc-800 flex gap-2">
        ...
        <button onClick={() => send(input)} disabled={!input.trim() || loading}
          className="w-9 h-9 rounded-xl bg-emerald-500 hover:bg-emerald-400 disabled:opacity-40
                     text-zinc-950 flex items-center justify-center transition text-base font-bold">
          ↑
        </button>
      </div>
    </div>
...
const FAQS = [
  ...
  { q: "Do we need to replace our current tools?", a: "No. PivotOps sits on top of what you already run. The goal in an early pilot is to prove the time-to-hire compression — not force a tool migration before you've seen results." },
    ...
    <div className="bg-zinc-950 text-white min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        ...
        <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-10 max-w-2xl">
          What your team's day looks like before and after.
        </h2>
        <BeforeAfter />
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          <div>
            ...
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight">
              Every day a role stays open is margin handed directly to a competitor who moved faster.
            </h2>
          </div>
          <div className="space-y-4 text-zinc-400 leading-relaxed">
            <p>The applicants are there. They get lost in the gap between submission and decision — a job opens with no clear owner, applications sit in a spreadsheet nobody checks, and an approval waits on a manager's WhatsApp reply that never comes.</p>
            <p>You've likely already bought an ATS. You're probably still running the real coordination — who owns what, who approved what, who's following up — across email, spreadsheets, and group chats. <span className="text-white">The tool was never the problem. The workflow was.</span></p>
          </div>
        </div>
      </section>
      ...
      <section id="how-it-works" className="max-w-6xl mx-auto px-6 py-20 border-t border-zinc-900">
        ...
        <h2 className="text-2xl md:text-3xl font-bold tracking-tight max-w-2xl mb-10">
          Five steps. One continuous system.
        </h2>
      ...
      {/* Features */}
      <section className="max-w-6xl mx-auto px-6 py-20 border-t border-zinc-900">
        <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-zinc-500 mb-4">What's built in</p>
      ...
      <section className="border-t border-zinc-900">
        <div className="max-w-6xl mx-auto px-6 py-24 text-center">
          ...
          <p className="text-zinc-400 max-w-xl mx-auto text-lg mb-10">
            No migration required. No six-month implementation. Bring your current pipeline and we'll show you the compression in weeks.
          </p>
      ...
      <footer className="border-t border-zinc-900">
        <div className="max-w-6xl mx-auto px-6 py-10 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-zinc-500">
          <div className="flex items-center gap-2">
            <PivotOpsLogo size={20} />
            <span className="font-semibold text-zinc-400">PivotOps</span>
          </div>
          <p>Workforce operations, run as one system.</p>
          <div className="flex items-center gap-4 text-xs">
            <a href="/login" className="hover:text-white transition">Sign in</a>
            <a href="/login?mode=signup" className="hover:text-white transition">Get started</a>
          </div>
        </div>
      </footer>
