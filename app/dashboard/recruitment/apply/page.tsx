...
import {
  ...
  Loader2, ChevronRight, Copy, ExternalLink,
} from "lucide-react";
...
const ROLES = [
  ...
  "Physician / Doctor", "Allied Health Professional",
  "Locum Tenens", "Clinical Therapist", "Pharmacist",
  "Radiographer", "Physiotherapist", "Healthcare Administrator", "Other",
];
...
export default function ApplicationPage() {
  ...
  const [copiedLink,  setCopiedLink]  = useState<"register" | "portal" | null>(null);
  ...
  const canProceedStep2 = (role && role !== "Other") || customRole.trim();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Use a temp ID for resume upload path — will be replaced by real candidateId
      ...
      // Build portal links directly from the candidateId returned by the API
      const base = window.location.origin;
      const registerLink = `${base}/candidate/register?candidateId=${data.candidateId}&tenantId=${tenantId}`;
      const portalLink   = `${base}/candidate/portal?candidateId=${data.candidateId}&tenantId=${tenantId}`;
  ...
  const copyToClipboard = async (text: string, which: "register" | "portal") => {
        ...
        <div className="w-full max-w-lg space-y-5">
          ...
          <div className="text-center space-y-3">
            <div className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center text-4xl
              ${isInterview ? "bg-emerald-500/20" : isReview ? "bg-amber-500/20" : "bg-zinc-800"}`}>
            ...
            <div>
              ...
              <p className="text-zinc-400 text-sm mt-1 leading-relaxed">
                {isInterview
                  ...
                  : isReview
                    ? "Your application is under review by our recruitment team."
                    : "Thank you for your interest. We'll keep your details on file."}
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            ...
            <div className="flex items-center gap-4 mb-3">
              <div className={`text-4xl font-bold ${
                isInterview ? "text-emerald-400" : isReview ? "text-amber-400" : "text-red-400"
              }`}>
                {result.score}
              </div>
              <div>
              ...
              <span className={`ml-auto text-xs px-3 py-1.5 rounded-full font-semibold border
            ...
            <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-1000
                  ${isInterview ? "bg-emerald-500" : isReview ? "bg-amber-500" : "bg-zinc-600"}`}
                style={{ width: `${result.score}%` }}
              />
            </div>
          </div>

          {/* Portal links — always shown so recruiter can share or candidate can self-navigate */}
          {!isRejected && (
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5 space-y-4">
              <div className="flex items-center gap-2">
                <CheckCircle2 size={14} className="text-emerald-400" />
                <h2 className="text-sm font-semibold text-white">Next Step — Create Compliance Account</h2>
              </div>
              <p className="text-xs text-zinc-500 leading-relaxed">
                Share this registration link with the candidate so they can create their account
                and upload their compliance credentials.
              </p>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2.5">
                  ...
                  <button
                    onClick={() => copyToClipboard(result.registerLink, "register")}
                    className="flex items-center gap-1 px-2 py-1 rounded-lg bg-zinc-700
                               hover:bg-zinc-600 text-xs text-zinc-300 transition flex-shrink-0"
                  >
                    {copiedLink === "register" ? <CheckCircle2 size={11} className="text-emerald-400" /> : <Copy size={11} />}
                    {copiedLink === "register" ? "Copied!" : "Copy"}
                  </button>
                  <a href={result.registerLink} target="_blank" rel="noreferrer"
                    className="w-7 h-7 flex items-center justify-center rounded-lg bg-zinc-700
                               hover:bg-zinc-600 text-zinc-400 transition flex-shrink-0">
                    <ExternalLink size={12} />
                  </a>
                </div>
              </div>

              {/* Portal link (after account exists) */}
              <div>
                <label className="text-[10px] text-zinc-600 uppercase tracking-wider mb-1.5 block">
                  Portal Link (after account creation)
                </label>
                <div className="flex items-center gap-2 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2.5">
                  <Link2 size={12} className="text-zinc-500 flex-shrink-0" />
                  <span className="text-xs text-zinc-300 truncate flex-1 font-mono">
                  ...
                  <button
                    onClick={() => copyToClipboard(result.portalLink, "portal")}
                    className="flex items-center gap-1 px-2 py-1 rounded-lg bg-zinc-700
                               hover:bg-zinc-600 text-xs text-zinc-300 transition flex-shrink-0"
                  >
                    {copiedLink === "portal" ? <CheckCircle2 size={11} className="text-emerald-400" /> : <Copy size={11} />}
              ...
              <p className="text-[10px] text-zinc-600">
                The candidate ID is embedded in the link — no manual copying required.
              </p>
            </div>
          )}

          <p className="text-center text-xs text-zinc-700">
            {fullName} · {new Date().toLocaleDateString()}
          </p>
        </div>
      </div>
    );
        ...
        <div className="w-full max-w-2xl mx-auto space-y-6">
          ...
          <div className="text-center">
            ...
            <p className="text-zinc-400 text-sm mt-2">
              Scored instantly by Xavier AI. Qualified candidates receive a compliance portal link immediately.
            </p>
          </div>
              ...
              <div key={s.n} className="flex items-center gap-2 flex-1">
                <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center
                  ...
                  ${step > s.n ? "bg-emerald-500 text-white" :
                    step === s.n ? "bg-indigo-600 text-white" : "bg-zinc-800 text-zinc-600"}`}>
                  {step > s.n ? "✓" : s.n}
                </div>
                <span className={`text-xs hidden sm:block ${step === s.n ? "text-white" : "text-zinc-600"}`}>
                  {s.label}
                </span>
                {i < steps.length - 1 && (
              ...
              <>
                ...
                <div className="rounded-xl bg-indigo-500/5 border border-indigo-500/15 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Brain size={14} className="text-indigo-400" />
                    <span className="text-xs font-semibold text-indigo-400">Xavier AI Scoring</span>
                  </div>
            ...
            {step === 3 && (
              <>
                ...
                <p className="text-zinc-500 text-sm">
                  A strong cover letter significantly improves your score.
                </p>
                ...
                <div>
                  <label className={labelCls}>
                    ...
                    <span className={`ml-auto text-[10px] ${
                      ...
                      {coverLetter.length > 300 ? " · +15pts" :
                       coverLetter.length > 100 ? " · +8pts"  : " · +3pts"}
                    </span>
                  </label>
                  <textarea
                    value={coverLetter}
          ...
          <p className="text-center text-xs text-zinc-700">
            Your application is scored instantly by Xavier AI. Qualified candidates receive
            a compliance portal link immediately after submission.
          </p>
        </div>
      </div>
    </>
