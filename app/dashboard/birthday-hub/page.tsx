...
import { Cake, Gift, Calendar as CalendarIcon, Settings as SettingsIcon, User, Loader2, ChevronLeft, ChevronRight, Sparkles, Send } from "lucide-react";
...
export default function BirthdayHubPage() {
  const [tab, setTab] = useState<"overview" | "calendar" | "mine" | "messages" | "cards" | "settings">("overview");
  ...
  const nameMap = useMemo(() => {
    ...
    feed.forEach((r) => { m[r.employee_id] = r.full_name ?? "Employee"; });
    return m;
  }, [feed]);
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <nav className="flex gap-1 mb-6 border-b border-zinc-800 overflow-x-auto" aria-label="Birthday Hub sections">
          {[
            ...
            { k: "messages", n: "Messages", icon: <Send size={13} /> },
            { k: "cards", n: "Cards", icon: <Sparkles size={13} /> },
        ...
        {loading ? (
          ...
          <>
            ...
            {tab === "messages" && me && <MessagesTab me={me} names={nameMap} />}
            {tab === "cards" && me && <CardsTab me={me} names={nameMap} companyName={companyName} />}
...
function SettingsTab({ tenantId, companyName }: { tenantId: string; companyName: string }) {
  // Hiding the PivotOps footer is a paid-plan entitlement. The gate is the
  // existing billing matrix - not a second permission system.
  const sub = useSubscription(tenantId);
  const canWhiteLabel = sub.features?.whiteLabel === true;
    ...
    <div className="max-w-md space-y-5">
      ...
      <div className="space-y-2.5">
        {[
          ...
          const locked = row.k === "show_powered_by" && !canWhiteLabel;
          return (
          <label key={row.k} className={"flex items-start gap-2.5 " + (locked ? "opacity-60 cursor-not-allowed" : "cursor-pointer")}>
            <input type="checkbox" checked={locked ? true : !!s[row.k]} disabled={locked} onChange={(e) => set(row.k, e.target.checked)}
              className="mt-0.5 accent-emerald-500" />
            <span>
              <span className="text-xs">{row.t}</span>
              <span className="block text-[10px] text-zinc-500">{locked ? "Included on the Free Trial. Upgrade your plan in Settings to hide the footer." : row.d}</span>
            </span>
          </label>
          );
        })}
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <p className="text-[10px] text-zinc-600 mt-1">Not applied yet - the birthday job runs once daily at 08:00 UTC. Honouring this hour needs an hourly schedule, which requires a Vercel Pro plan.</p>
        </div>
        <div>
