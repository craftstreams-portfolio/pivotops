"use client";
import SupportInquiryPanel from "./SupportInquiryPanel";
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        if (regular.length > 0 && !activeChannel) {
          // ?channel=<id> lets the inquiry alert email open the right
          // conversation directly instead of dropping you on the default.
          const wanted = typeof window !== "undefined"
            ? new URLSearchParams(window.location.search).get("channel") : null;
          const target = (wanted && regular.find((c: any) => c.id === wanted)) || regular[0];
          setActiveChannel(target as Channel);
          markRead(target.id);
        }
        setLoading(false);
      });
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        {!activeChannel ? (
          ...
          <>
            ...
            <SupportInquiryPanel channelId={activeChannel?.id ?? null} />
                    ...
                    <div className="flex items-end gap-2">
                      <div className="flex-1 bg-zinc-800/60 border border-zinc-700/60 rounded-2xl
                        ...
                        <MentionInput
                          ...
                          onChange={(v) => {
                            mention.setValue(v);
                            // Support inquiries also notify the website visitor.
                            // Throttled: the RPC is cheap but the visitor only
                            // needs to know someone is there, not every keystroke.
                            if (activeChannel && Date.now() - supportTypingRef.current > 2500) {
                              supportTypingRef.current = Date.now();
                              supabase.rpc("support_agent_typing", { p_channel: activeChannel.id }).then(() => {}, () => {});
                            }
