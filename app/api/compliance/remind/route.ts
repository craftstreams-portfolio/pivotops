...
export const POST = withSecurity(
  async (_req, { auth }) => {
    ...
    const { data: rejectedDocs, error: docsErr } = await getAdmin().from("compliance_docs").select("*").eq("status", "rejected").eq("tenant_id", tenantId);
    ...
    for (const [candidateId, docs] of Object.entries(byCandidateId)) {
      ...
      const { data: account } = await getAdmin().from("candidate_accounts").select("full_name, email, tenant_id").eq("candidate_id", candidateId).single();
      ...
      await getAdmin().from("xavier_notifications").insert({ tenant_id: tenantId, candidate_id: candidateId, stage: "auto_reject", message, type: reminderCount >= 4 ? "alert" : "warning", read: false, created_at: new Date().toISOString() });
      logger.info("Compliance reminder sent", { candidateId, reminderCount, tenantId });
      const now = new Date().toISOString();
      await getAdmin().from("compliance_docs").update({ last_reminder_at: now, reminder_count: reminderCount, updated_at: now }).eq("candidate_id", candidateId).eq("status", "rejected").eq("tenant_id", tenantId);
      reminded++;
    }
