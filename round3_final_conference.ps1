# ══════════════════════════════════════════════════════════════════
# Round 3: transferHost/assignSpeaker functions, notify channel,
# Participants panel render. Applied bottom-to-top so anchors don't shift.
# ══════════════════════════════════════════════════════════════════

$path = "app\dashboard\meetings\page.tsx"
$lines = Get-Content $path
Copy-Item $path "$path.bak6" -Force
Write-Host "Backup saved: $path.bak6" -ForegroundColor Cyan

# ── STEP 1 (bottom-most): Participants panel render, insert after line 1326 ($)}") ──
$insertAfter = 1326
$panelBlock = @(
'',
'      {showParticipants && (',
...
Write-Host "[OK] Step 1: Participants panel render inserted" -ForegroundColor Green
...
'      await admin.from("meetings").update({ host_user_id: newHostUserId }).eq("id", activeMeeting.id);',
'      await admin.from("meeting_participants").update({ participant_role: "participant" })',
'        .eq("meeting_id", activeMeeting.id).eq("participant_role", "host");',
'      await admin.from("meeting_participants").update({ participant_role: "host" })',
...
# ── STEP 3: notify channel subscription, insert after line 671 (participants subscription close) ──
$insertAfter = 671
$notifyBlock = @(
'',
...
# ── STEP 4 (top-most): notifyChanRef declaration, insert after line 484 (engineRef) ──
$insertAfter = 484
...
Write-Host "`nAll round-3 patches written." -ForegroundColor Cyan
